...
# Dependency to validate password length
def validate_password_length(password: Annotated[constr(min_length=8), Form()]) -> str:
    """
    Validates that the password has a minimum length and is not longer than 72 bytes (bcrypt limit).
    To be used in signup/password change routes.
    """
    if len(password.encode('utf-8')) > 72:
        # This provides a more accurate validation against bcrypt's 72-byte limit.
        raise HTTPException(
