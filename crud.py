...
async def create_user(db: AsyncSession, user: schemas.UserCreate, *, is_active: bool = False, is_verified: bool = False, account_number: str | None = None):
...
async def approve_kyc_submission(db: AsyncSession, submission_id: int, approver_id: int | None = None):
...
async def reject_kyc_submission(db: AsyncSession, submission_id: int, reason: str | None = None):
    result = await db.execute(select(models.KYCSubmission).filter(models.KYCSubmission.id == submission_id))
    submission = result.scalar_one_or_none()
    if not submission:
        return None
