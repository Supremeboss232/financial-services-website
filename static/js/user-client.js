...
(function(){
    async function fetchStats(){
        try{
            ...
            if(!data) return;
            document.getElementById('user-balance')?.textContent = data.balance?.toLocaleString?.() ?? data.balance ?? '0.00';
            document.getElementById('user-investments')?.textContent = data.investments?.toLocaleString?.() ?? data.investments ?? '0.00';
            document.getElementById('user-loans')?.textContent = data.loans?.toLocaleString?.() ?? data.loans ?? '0.00';
